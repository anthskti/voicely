...
export default function ScenesPage() {
  ...
  return (
    <div className="relative min-h-screen bg-[#262733] text-[#EDEFF1] flex flex-col overflow-hidden scanline-bg pb-[calc(5rem+env(safe-area-inset-bottom))] lg:pb-0">
      ...
      <header className="relative z-20 flex h-16 items-center justify-between border-b border-[#93BADF]/15 bg-[#1a1b24]/80 px-4 sm:px-6 backdrop-blur-xl">
        <div className="flex items-center gap-2 sm:gap-4 min-w-0">
          ...
          <span className="hidden sm:inline text-xs text-[#EDEFF1]/40">•</span>
          <span className="hidden sm:inline text-xs font-semibold text-[#93BADF] uppercase tracking-widest">
            SCENE SELECT
          </span>
        </div>
        ...
        <div className="flex items-center gap-4 text-xs shrink-0">
          {session?.user ? (
            <span className="hidden sm:inline text-[#708F7F] font-mono truncate max-w-[120px]">
      ...
      {/* Mobile: preview → horizontal strip → meta → CTA */}
      <main className="relative z-10 flex-1 w-full max-w-7xl mx-auto p-4 sm:p-6 lg:hidden">
      ...
      {/* Desktop: two-column osu! layout */}
      <main className="relative z-10 hidden lg:grid flex-1 max-w-7xl w-full mx-auto p-8 grid-cols-12 gap-8 items-stretch min-h-[calc(100vh-64px)]">
        <div className="col-span-6 flex flex-col justify-between glass-panel p-8 rounded-3xl border border-[#93BADF]/25 bg-[#1d1e27]/85 backdrop-blur-2xl shadow-2xl">
          <ScenePanelBody
            loading={loading}
            error={error}
            selectedScene={selectedScene}
