...
import { BookMarked, Clapperboard, Info, LogIn } from "lucide-react";
...
export function Sidebar() {
  ...
  const scenesActive = pathname.startsWith("/scenes") || pathname.startsWith("/studio");
  ...
  return (
    <>
      <aside className="hidden lg:flex w-64 bg-[#1a1b24]/90 border-r border-[#93BADF]/15 backdrop-blur-xl flex-col justify-between p-6 shrink-0 z-20">
        <div>
      ...
      <nav
        className="lg:hidden fixed bottom-0 inset-x-0 z-40 flex items-stretch border-t border-[#93BADF]/15 bg-[#1a1b24]/90 backdrop-blur-xl pb-[env(safe-area-inset-bottom)]"
        aria-label="Primary"
      >
        <TabLink href="/scenes" label="Scenes" active={scenesActive}>
          <Clapperboard className="h-5 w-5" strokeWidth={scenesActive ? 2.4 : 1.8} />
        </TabLink>
        <TabLink href="/#about" label="About" active={aboutActive}>
          <Info className="h-5 w-5" strokeWidth={aboutActive ? 2.4 : 1.8} />
        </TabLink>
        {signedIn ? (
          <TabLink href="/library" label="Library" active={libraryActive}>
            <BookMarked className="h-5 w-5" strokeWidth={libraryActive ? 2.4 : 1.8} />
          </TabLink>
        ) : (
          <TabLink href="/login" label="Log in" active={loginActive}>
            <LogIn className="h-5 w-5" strokeWidth={loginActive ? 2.4 : 1.8} />
          </TabLink>
        )}
      </nav>
    </>
  ...
  return (
    <Link
      ...
      className={`flex flex-1 flex-col items-center justify-center gap-1 py-2.5 text-[10px] font-semibold uppercase tracking-wider transition-colors ${
        active ? "text-white" : "text-[#EDEFF1]/50"
      }`}
    >
      <span className={active ? "text-[#93BADF]" : "text-[#EDEFF1]/40"}>{children}</span>
      {label}
    </Link>
  );
}
